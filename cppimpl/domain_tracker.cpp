...
void DomainTracker::update(const std::vector<std::vector<int>> &new_groups,
  ...
  for (size_t i = 0; i < previous_groups.size(); ++i) {
    for (size_t j = 0; j < new_groups.size(); ++j) {
      ...
      float overlap =
          BVHBuilder::calc_overlap(previous_nodes[i]->aabb, new_nodes[j]->aabb);
...
DomainTracker::match_domains(const std::vector<std::vector<int>> &new_groups,
                             const std::vector<const Node *> &new_nodes) {
  ...
  for (size_t j = 0; j < new_groups.size(); ++j) {
    if (std::find(assignment.begin(), assignment.end(), j) ==
        assignment.end()) {
