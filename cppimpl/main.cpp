...
int main(int argc, char *argv[]) {
  try {
    ...
    for (int frame = 0; frame < data_handler.num_frames(); ++frame) {
      ...
      std::cout << "Frame " << frame << ": Loaded " << aabbs.size() << " AABBs"
                << std::endl;
      float current_time = data_handler.get_frame_time(frame);

      // Print current simulation time
      std::cout << "Current simulation time: " << current_time << " seconds"
                << std::endl;
      ...
      auto groups =
          builder.get_subdomains_greedy(bvh_root.get(), num_subdomains);

      // Match domains
      auto ordered_groups = domain_tracker.match_domains(
          groups, builder.get_group_nodes(bvh_root.get(), groups));
      ...
      std::cout << "Created " << groups.size()
                << " subdomain groups:" << std::endl;
      for (size_t i = 0; i < groups.size(); ++i) {
        std::cout << "Group " << i << " has " << groups[i].size() << " objects"
                  << std::endl;
      }
    }
