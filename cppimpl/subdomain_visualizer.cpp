...
SubdomainVisualizer::SubdomainVisualizer() { init_gl_window(); }
...
void SubdomainVisualizer::init_gl_window() {
  ...
  window =
      glfwCreateWindow(1200, 800, "Subdomain Visualization", nullptr, nullptr);
  if (!window) {
    glfwTerminate();
...
void SubdomainVisualizer::visualize(
  ...
  if (!aabbs.empty()) {
    ...
    std::cout << "Min: (" << aabbs[0].min[0] << ", " << aabbs[0].min[1] << ", "
              << aabbs[0].min[2] << ")" << std::endl;
    std::cout << "Max: (" << aabbs[0].max[0] << ", " << aabbs[0].max[1] << ", "
              << aabbs[0].max[2] << ")" << std::endl;
  }
  ...
  glRotatef(-135.0f, 0.0f, 1.0f,
            0.0f); // changed azimuth to -135 for better orientation

  // Predefined color palette
  const float colors[][3] = {{0.12f, 0.47f, 0.71f}, {0.17f, 0.63f, 0.17f},
                             {0.54f, 0.21f, 0.58f}, {0.89f, 0.47f, 0.76f},
                             {0.80f, 0.73f, 0.13f}, {0.64f, 0.08f, 0.18f}};

  BVHBuilder builder(aabbs);
  auto ordered_groups = domain_tracker.match_domains(
      groups, builder.get_group_nodes(nullptr, groups));

  // Draw AABBs by group
  for (size_t group_idx = 0; group_idx < ordered_groups.size(); ++group_idx) {
    const auto &group = ordered_groups[group_idx];
